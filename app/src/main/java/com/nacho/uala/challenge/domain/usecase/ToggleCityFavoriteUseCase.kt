...
@Singleton
class ToggleCityFavoriteUseCase @Inject constructor(
    private val repository: CityRepository,
    @param:IoDispatcher private val dispatcher: CoroutineDispatcher
){

    suspend operator fun invoke(city: City): Result<Unit> = withContext(dispatcher) {
        repository.toggleCityFavorite(city)
    }
}